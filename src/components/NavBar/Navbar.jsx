        ...
        <AnimatePresence>
            {isHover === true ? (
                ...
                <>
                    ...
                    <motion.div onClick={() => setIsHover(false)} variants={dropdownVariants} initial='hidden' animate='visible' whileHover='visible' exit='hidden' className="absolute right-[-10px] bottom-[-70px] bg-quinary rounded-xl px-6 py-2 mt-5">
                        <Link to={'/login'} className="text-xl border-b border-tertiary">Login</Link>
                        <Link to={'/register'} className="block text-xl mt-2">Register</Link>
                    </motion.div>
                </>
            ) : (
            ""
